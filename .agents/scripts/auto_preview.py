...
def get_start_command(root, port=3000):
    ...
    if pkg_file.exists() and has_npm:
...
def status_server():
        ...
        try:
            ...
            if is_running(pid):
                ...
                if PORT_FILE.exists():
                    ...
                    # Discover port from lsof
                    try:
                        out = subprocess.check_output(["lsof", "-Pan", "-p", str(pid), "-i"], text=True)
    ...
    if running:
        ...
        print(f"📱 Network: http://{lan_ip}:{port} (for mobile on same WiFi)")
        print(f"📝 Logs: {LOG_FILE}")
    else:
